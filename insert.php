...
$select = mysqli_query($conn,"SELECT * FROM Usertable WHERE email='$Email' and username='$Username'");
$run    = mysqli_fetch_array($select);
if($run>0)
{
	
	if($Username == $run['username'] && $Email == $run['email']){
		echo 4;
	}
	else if($Email == $run['email']){
	   echo 2;	
	}
	else if($Username == $run['username']){
		echo 3;
	}
